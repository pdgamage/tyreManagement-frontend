  ...
  const stats = useMemo(() => {
    ...
    const workflowRequests = requests.filter((r) => {
      if (role === "supervisor") {
        ...
        return (
          r.status?.toLowerCase().trim() === "user requested tire" ||
    ...
    const pendingRequests = requests.filter((r) => {
      if (role === "supervisor") {
        return r.status?.toLowerCase().trim() === "user requested tire";
      } else if (role === "technical-manager") {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-gray-500 text-sm font-medium">
            {role === "customer-officer" ? "Pending Orders" : "User Requested tire Review"}
          </h3>
          <div className="mt-2 flex items-center">
            <span className="text-3xl font-bold text-gray-900">
