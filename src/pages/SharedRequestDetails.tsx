...
const SharedRequestDetails = () => {
  ...
  return <TireRequestDetailsCard request={request} fromInquiry={state?.fromInquiry} />;
          ...
          <hr />
          {/* Supervisor Notes - Only show if supervisor or if notes exist */}
          {(isSupervisor || (request.status !== "pending" && request.supervisor_notes)) && (
            <div>
              <label className="block mb-1 font-semibold text-gray-700">
                Supervisor Notes {isSupervisor && request.status === "pending" && <span className="text-red-500">*</span>}
              </label>
              <textarea
                ...
                rows={3}
                readOnly={!isSupervisor || request.status !== "pending"} 
              />
              {notesError && (
